    ...
    ILogger<ReviewService> logger)
{
    public async Task<bool> IsEnabledAsync() =>
        await parameters.GetBoolAsync(VentaMapParameterService.ReviewsEnabled);
        ...
        string baseUrl)
    {
        ...
        if (await parameters.GetBoolAsync(VentaMapParameterService.ReviewEmailsEnabled))
        {
        ...
        bool declined)
    {
        ...
        var roleEnabledKey = reviewerIsAdvertiser
            ? VentaMapParameterService.CounterpartyReviewsEnabled
            : VentaMapParameterService.AdvertiserReviewsEnabled;
        ...
        if (operation.Reviews.Select(x => x.ReviewerUserId).Distinct().Count() >= 2)
        {
            var delayDays = await parameters.GetIntAsync(
                VentaMapParameterService.ReviewPublicationDelayDays, 7, 0, 90);
    ...
    {
        if (!await IsEnabledAsync()
            || !await parameters.GetBoolAsync(VentaMapParameterService.DisplayExistingReviewsEnabled))
        {
            return null;
        }
        ...
        var responseDays = await parameters.GetIntAsync(
            VentaMapParameterService.ReviewResponseDeadlineDays, 14, 1, 90);
        var delayDays = await parameters.GetIntAsync(
            VentaMapParameterService.ReviewPublicationDelayDays, 7, 0, 90);
