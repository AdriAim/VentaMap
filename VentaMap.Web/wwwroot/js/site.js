(() => {
  ...
  const systemLoadingStorageKey = "ventamap:system-loading";
  let suppressNextBeforeUnloadSystemLoading = false;
  const lastClickedGalleryPublicationStorageKey = "ventamap:last-clicked-gallery-publication";
  const pendingAuthActionStorageKey = "ventamap:pending-auth-action";
  const favoriteLastListStorageKey = "ventamap:last-favorite-list-id";
  const likedPublicationsStorageKey = "ventamap:liked-publications";
  const publicationOpenModeStorageKey = "ventamap:publication-open-mode";
  const NAVIGATION_LOCALITY_COOKIE = "ventamap_nav_locality_id";
  const HEADER_PUBLICATION_GROUPS_COOKIE = "ventamap_header_groups";
  ...
  const chatConfig = window.__VENTAMAP_CHAT_CONFIG || {};
  ...
  function detailDebugLog(label, data) {
    ...
    if (typeof data === "undefined") {
      console.log(`[ventamap-detail] ${label}`);
      return;
    }
    console.log(`[ventamap-detail] ${label}`, data);
  }
  ...
  async function loadApiPage() {
    ...
    try {
      ...
      if (ventamapFlashMessage) {
        const banner = document.createElement("div");
        banner.className = "status-banner";
        banner.textContent = ventamapFlashMessage;
        host.prepend(banner);
        ventamapFlashMessage = "";
      }
  ...
  function wireReportForm() {
    ...
    form.addEventListener("submit", async event => {
      ...
      if (!response.ok) {
        ...
        ventamapFlashMessage = result?.message || "No se pudo enviar la denuncia.";
        await loadApiPage();
        return;
      }

      ventamapFlashMessage = result.message || "La denuncia fue enviada.";
  ...
  function wireFavoriteModal() {
    ...
    form.addEventListener("submit", async event => {
      ...
      if (!response.ok) {
        ...
        ventamapFlashMessage = result?.message || "No se pudo guardar en favoritos.";
        close();
        await loadApiPage();
        return;
      }
      ...
      ventamapFlashMessage = result?.message || "Guardado en favoritos.";
      close();
      await loadApiPage();
    });
  ...
  async function openFavoriteModalByPayload(payload = {}) {
    ...
    if (!response.ok) {
      ...
      ventamapFlashMessage = result?.message || "Tenes que iniciar sesion para usar favoritos.";
      await loadApiPage();
      return;
    }
  ...
  window.__ventamapBuildGalleryCard = buildGalleryCard;
  window.__ventamapWireGalleryCards = wireGalleryCards;
  window.__ventamapWireFavoriteActions = wireFavoriteActions;
  window.__ventamapWireReportForm = wireReportForm;
  window.__ventamapSyncMobileGalleryVideoAutoplay = syncMobileGalleryVideoAutoplay;
  ...
  function buildChatNetworkErrorMessage(url, error) {
    ...
    if (origin && isLocalhostOrigin(origin)) {
      return `No se pudo conectar al servicio de chat en ${origin}. En debug inicia VentaMap.ChatService junto con VentaMap.Web.`;
    }
