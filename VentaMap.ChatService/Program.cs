...
var ventamapConnectionString = builder.Configuration.GetConnectionString("VentaMapConnection")
    ?? builder.Configuration.GetConnectionString("DefaultConnection")
    ?? throw new InvalidOperationException("Falta ConnectionStrings:VentaMapConnection.");
...
builder.Services.AddDbContext<VentaMapLookupDbContext>(options =>
{
    options.UseMySql(ventamapConnectionString, ServerVersion.AutoDetect(ventamapConnectionString));
});
